            ...
            <table class="table rit-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th>Name</th>
                        <th data-breakpoints="lg">Gender</th>
                    ...
                    @foreach($users as $key => $user)
                        @if ($user != null)
                            <tr>
                                ...
                                <td>{{$user->email}}</td>
                                <td>{{$user->phone}}</td>
                                <td>@if(is_numeric($user->verification_code)){{$user->verification_code}}@endif</td>


                                <td class="text-right">
                                    <a href="{{route('users.login', encrypt($user->id))}}" class="btn btn-soft-primary btn-icon btn-circle btn-sm" title="Log in as this Customer">
                                        <i class="las la-edit"></i>
                                    </a>
                                    @if($user->banned != 1)
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm" onclick="confirm_ban('{{route('users.ban', $user->id)}}');" title="Ban this Customer">
                                        <i class="las la-user-slash"></i>
                                    </a>
                                    @else
                                    <a href="#" class="btn btn-soft-success btn-icon btn-circle btn-sm" onclick="confirm_unban('{{route('users.ban', $user->id)}}');" title="Unban this Customer">
